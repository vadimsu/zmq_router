...
#include <specific_includes/linux/time.h>
//#include <linux/slab.h>

#include <specific_includes/net/icmp.h>
#include <specific_includes/net/inet_common.h>
...
void tcp_init_sock(struct sock *sk)
{
	...
	LINUX_INIT_LIST_HEAD(&tp->tsq_node);
...
#endif
uint64_t sk_stream_alloc_skb_failed = 0;
...
//		tp->ucopy.task = NULL;
	...
	release_sock(sk);
#ifdef OPTIMIZE_TCP_RECEIVE
	msg->msg_iov->head = tp->ucopy.iov.head;
	msg->msg_iov->tail = tp->ucopy.iov.tail;
	tp->ucopy.iov.head = NULL;
	tp->ucopy.iov.tail = NULL;
	copied = tp->ucopy.iov.len;
	msg->msg_iov->len = copied;
	tp->ucopy.iov.len = 0;
	tp->ucopy.len = sk->sk_rcvbuf;
#endif
	return copied;
